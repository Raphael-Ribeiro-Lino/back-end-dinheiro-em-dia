...
public class FindByIdIncomeCategoryControllerTest {
	...
	@Test
	void quando_buscarCategoriaReceitaPorIdSemToken_Entao_RetornaErroAcessoNegado() throws Exception {
		ResultActions result = mvc.findWithUnauthorized(uri);
		result.andExpect(jsonPath("$.[?(@.message == 'Acesso Negado!')]").exists());
	}
	
	@Test
	void quando_buscarCategoriaReceitaPorIdTokenInvalido_Entao_RetornaErroTokenInvalido() throws Exception {
		ResultActions result = mvc.findWithUnauthorized(uri, token+1);
		result.andExpect(jsonPath("$.[?(@.message == 'Token inválido!')]").exists());
	}
	
	@Test
	void quando_buscarCategoriaReceitaPorId_Entao_RetornaOk() throws Exception {
